...
def polyline(samples, x_key, y_key, scale):
    ...
    for sample in samples:
        x, y = scale(sample[x_key], sample[y_key])
        points.append(f"{x:.2f},{y:.2f}")
    return " ".join(points)


def write_svg(path, title, series, x_key, y_key, x_label, y_label):
...
def main():
    build = Path("build")
    ideal = read_samples(build / "projectile_ideal.csv")
    drag = read_samples(build / "projectile_drag.csv")
    series = {"ideal": ideal, "drag": drag}

    write_svg(build / "projectile_trajectory.svg", "Projectile Trajectory", series, "x_m", "y_m", "x position (m)", "y position (m)")
    write_svg(build / "projectile_energy.svg", "Total Mechanical Energy", series, "time_s", "total_energy_j", "time (s)", "total energy (J)")
    print("Wrote build/projectile_trajectory.svg")
    print("Wrote build/projectile_energy.svg")
